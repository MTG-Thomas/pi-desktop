...
function sessionKey(ctx: unknown): string | null {
  try {
    const sm = (
      ctx as { sessionManager?: { getSessionFile?: () => string | null; getSessionId?: () => string | null } }
    )?.sessionManager
...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      try {
        sessionFile =
          (
            ctx as unknown as { sessionManager?: { getSessionFile?: () => string | null } }
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params) {
      ...
      const filtered = all.filter((e) => (!thread || e.threadId === thread) && (!topic || e.topic === topic))
  ...
  pi.on('session_start', async (_event, ctx) => {
    try {
      ...
      if (all.length > 0) {
        ctx.ui.notify(
          `bus: ${unread} unpolled post(s), ${urgent} urgent. Call bus_read at turn start and before finishing.`,
          'info',
        )
      }
    } catch {
